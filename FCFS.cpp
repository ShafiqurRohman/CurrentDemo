...
int main() {
    ...
    // Output the results
    cout << "Process ID\tArrival Time\tBurst Time\tWaiting Time\tCompletion Time" << endl;
    for (int i = 0; i < n; i++) {
        cout << p[i].pid << "\t\t" << p[i].arrivalTime << "\t\t" << p[i].burstTime << "\t\t" << p[i].waitingTime << "\t\t" << p[i].completionTime << endl;
    }
    return 0;
}
